...
// The one full-date format every non-chart date display shares — "2024-01-05" -> "Jan 05, 2024".
// Same string-slice approach as formatMonthYear, for the same timezone-safety reason.
export function formatFullDate(dateStr: string): string {
  const [year, month, day] = dateStr.split("-");
  return `${SHORT_MONTHS[Number(month) - 1]} ${day}, ${year}`;
}
